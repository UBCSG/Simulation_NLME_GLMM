...
# The viral load (in log10-scale) trajectories during treatment.
...
# The viral load (in log10-scale) trajectories during treatment for the first 10 days.
data.10days <- data.decay %>% 
  filter(time <= 10)
p1.10days <- ggplot(data.10days, aes(x=time, y=log10)) + 
  geom_point(aes(shape=factor(censor)),size=2, stroke=0) +
  geom_line(aes(group=PATIENT)) +
  scale_x_continuous("Day") + 
  scale_y_continuous(bquote("Viral load (in" ~ log[10]~"-scale)"))+
