...
time <- c(0.5, 0.7, 1.2, 1.8, 2.3, 2.9, 3.3, 4.8, 7, 10.2) #PATIENT #1
...
for (i in 1:num_sim){
  ...
  write.table(data_sim,"Data/data_reboundV.txt", sep = "," ,quote = FALSE,row.names = FALSE)
  
  # Fit viral Rebound model using Monolix.
  start.time <- Sys.time()
  data = list(dataFile = paste0('Data/data_reboundV.txt'),
  ...
  setIndividualParameterVariability(beta1 = TRUE, beta3 = TRUE)
  ...
  estimates_SAEM[i,] <- getEstimatedPopulationParameters()[1:5]
  ...
  tryCatch({
    model_lme4 <- nlmer(viral ~ nfun(day, beta1, beta2, beta3, beta4, beta5) ~ (beta1 + beta3| PATIENT), data_nlme, start = c(beta1 = 4, beta2 = 5.7, beta3 = 2.1, beta4 = 1.9, beta5 = 0.4))
    estimates_LME4[i, ] <- summary(model_lme4)$coefficients[, 1]
    SE_LME4[i, ] <- summary(model_lme4)$coefficients[, 2]
  ...
  write.csv(estimates_SAEM, "Results/ReboundV/estimates_SAEM.csv", row.names = FALSE)
  write.csv(estimates_NLME, "Results/ReboundV/estimates_NLME.csv", row.names = FALSE)
  write.csv(estimates_LME4, "Results/ReboundV/estimates_LME4.csv", row.names = FALSE)
  write.csv(SE_SAEM, "Results/ReboundV/SE_SAEM.csv", row.names = FALSE)
  write.csv(SE_NLME, "Results/ReboundV/SE_NLME.csv", row.names = FALSE)
  write.csv(SE_LME4, "Results/ReboundV/SE_LME4.csv", row.names = FALSE)
  write.csv(time_SAEM, "Results/ReboundV/time_SAEM.csv", row.names = FALSE)
  write.csv(time_NLME, "Results/ReboundV/time_NLME.csv", row.names = FALSE)
  write.csv(time_LME4, "Results/ReboundV/time_LME4.csv", row.names = FALSE)
}  


#======================Calculate MSE and bias===================
# Read the results file
estimates_SAEM <- read.csv("Results/ReboundV/estimates_SAEM.csv")
estimates_NLME <- read.csv("Results/ReboundV/estimates_NLME.csv")
estimates_LME4 <- read.csv("Results/ReboundV/estimates_LME4.csv")
SE_SAEM <- read.csv("Results/ReboundV/SE_SAEM.csv")
SE_NLME <- read.csv("Results/ReboundV/SE_NLME.csv")
SE_LME4 <- read.csv("Results/ReboundV/SE_LME4.csv")
time_SAEM <- read.csv("Results/ReboundV/time_SAEM.csv")$x
time_NLME <- read.csv("Results/ReboundV/time_NLME.csv")$x
time_LME4 <- read.csv("Results/ReboundV/time_LME4.csv")$x
...
result_SAEM <- cbind(true_value, estimates = sapply(estimates_SAEM, mean), se_m = sapply(SE_SAEM, mean), se_s = sapply(estimates_SAEM, sd), rMSE_SAEM,bias_SAEM,bias_perc_SAEM, Coverage_SAEM, NC = c(num_sim - nrow(estimates_SAEM), NA,NA, NA,NA), time = c(sum(time_SAEM), NA,NA, NA, NA))
