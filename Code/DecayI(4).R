...
time <- c(0.5,1.67,2.3,3,4.6,6.5,7.6,11.2,14.9,19.1) #PATIENT #1
...
for (i in 1:100){
  ...
  write.table(data_saem,"Data/data_saem_DecayI.txt", sep = "," ,quote = FALSE,row.names = FALSE)
  
  # Fit viral decay model using Monolix.
  start.time <- Sys.time()
  data = list(dataFile = paste0('Data/data_saem.txt'),
              headerTypes =c("id","time","observation","cens","limit"),
  ...
  SE_SAEM[i,] <- getEstimatedPopulationParameters()[4:6]
  end.time <- Sys.time()
  ...
  cat("Iteration:", i, "\n")
  start.time <- Sys.time()
  data_nlme <- groupedData(viral ~ day | PATIENT, data = data_nlme)
  ...
  }, error = function(e){print("Fail to converge")} 
  )
...
write.csv(estimates_SAEM, "Results/DecayI/estimates_SAEM.csv", row.names = FALSE)
write.csv(estimates_NLME, "Results/DecayI/estimates_NLME.csv", row.names = FALSE)
write.csv(estimates_LME4, "Results/DecayI/estimates_LME4.csv", row.names = FALSE)
write.csv(SE_SAEM, "Results/DecayI/SE_SAEM.csv", row.names = FALSE)
write.csv(SE_NLME, "Results/DecayI/SE_NLME.csv", row.names = FALSE)
write.csv(SE_LME4, "Results/DecayI/SE_LME4.csv", row.names = FALSE)
write.csv(time_SAEM, "Results/DecayI/time_SAEM.csv", row.names = FALSE)
write.csv(time_NLME, "Results/DecayI/time_NLME.csv", row.names = FALSE)
write.csv(time_LME4, "Results/DecayI/time_LME4.csv", row.names = FALSE)


#======================Calculate MSE and bias===================
# Read the results file
estimates_SAEM <- read.csv("Results/DecayI/estimates_SAEM.csv")
estimates_NLME <- read.csv("Results/DecayI/estimates_NLME.csv")
estimates_LME4 <- read.csv("Results/DecayI/estimates_LME4.csv")
SE_SAEM <- read.csv("Results/DecayI/SE_SAEM.csv")
SE_NLME <- read.csv("Results/DecayI/SE_NLME.csv")
SE_LME4 <- read.csv("Results/DecayI/SE_LME4.csv")
time_SAEM <- read.csv("Results/DecayI/time_SAEM.csv")
time_NLME <- read.csv("Results/DecayI/time_NLME.csv")
time_LME4 <- read.csv("Results/DecayI/time_LME4.csv")
...
result_SAEM <- cbind(true_value, estimates = sapply(estimates_SAEM, mean), se = sapply(SE_SAEM, mean),rMSE_SAEM,bias_SAEM,bias_perc_SAEM, Coverage_SAEM, NC = c(0, NA,NA), time = c(sum(time_SAEM), NA, NA))
...
result_NLME <- cbind(true_value,estimates = sapply(estimates_NLME, mean), se = sapply(SE_NLME, mean),rMSE_NLME,bias_NLME,bias_perc_NLME, Coverage_NLME, NC = c(num_sim-nrow(estimates_NLME), NA, NA), time = c(sum(na.omit(time_NLME)), NA, NA))
...
for (i in seq_along(true_value)){
  rMSE_LME4[i]=sqrt(sum((estimates_LME4[,i]-true_value[i])^2)/num_sim)/true_value[i]
  bias_LME4[i]=sum(estimates_LME4[,i]-true_value[i])/num_sim
  bias_perc_LME4[i]=bias_LME4[i]/true_value[i]
}
Coverage_LME4 <- sapply(CI_LME4, mean)
result_LME4 <- cbind(true_value,estimates = sapply(estimates_LME4, mean), se = sapply(SE_LME4, mean),rMSE_LME4,bias_LME4,bias_perc_LME4, Coverage_LME4, NC = c(0, NA, NA), time = c(sum(time_LME4), NA, NA))
rownames(result_LME4)=c("p1","lambda1","p2")

# Latex Table
table_names <- c( "Method", "Time (s)", "NC","Parameter","True Value", "Estimate", "SE", "Bias (%)","rMSE (%)","Coverage")
...
table$SE <- c(result_SAEM[, 3], result_NLME[,3], result_LME4[,3])
table$`Bias (%)` <- c(result_SAEM[, 6]*100, result_NLME[,3]*100, result_LME4[,3]*100)
table$`rMSE (%)` <- c(result_SAEM[, 4]*100, result_NLME[,4]*100, result_LME4[,4]*100)
table$Coverage <-  c(result_SAEM[, 7]*100, result_NLME[,7]*100, result_LME4[,7]*100)
table$`Time (s)` <- c(result_SAEM[, 9], result_NLME[,9], result_LME4[,9])
table$NC <- c(result_SAEM[, 8], result_NLME[,8], result_LME4[,8])

latex_table<-xtable(table, type = "latex",align=c("ccccccccccc"))
digits(latex_table)<-c(0,3,1,0,1,1,3,3,3,3,2)
print(latex_table, file = "Table/DecayI.tex",include.rownames=FALSE,sanitize.text.function = function(x){x},hline.after = c(-1,0,9))
