...
num_patient <- 20  #number of patients
...
D_SAEM <- D_NLME <- D_LME4 <- data.frame(matrix(nrow=num_sim,ncol = 3))
...
for (i in 11:num_sim){
  ...
  write.table(data_sim,"Data/data_Decay_n20_ni5.txt", sep = "," ,quote = FALSE,row.names = FALSE)
  
  # Fit viral decay model using Monolix.
  start.time <- Sys.time()
  data = list(dataFile = paste0('Data/data_Decay_n20_ni5.txt'),
  ...
  }, warning = function(w) {estimates_NLME[i, ] <<- SE_NLME[i, ] <<- D_NLME[i, ] <<- sigma_NLME[i] <<- NA}, 
  message = function(m) {estimates_NLME[i, ] <<- SE_NLME[i, ] <<- D_NLME[i, ] <<- sigma_NLME[i]  <<-NA}, 
  error = function(e) {estimates_NLME[i, ] <<- SE_NLME[i, ] <<- D_NLME[i, ] <<- sigma_NLME[i]  <<- NA})
...
write.csv(estimates_SAEM, "Results/Decay_n20_ni5/estimates_SAEM.csv", row.names = FALSE)
write.csv(estimates_NLME, "Results/Decay_n20_ni5/estimates_NLME.csv", row.names = FALSE)
write.csv(estimates_LME4, "Results/Decay_n20_ni5/estimates_LME4.csv", row.names = FALSE)
write.csv(SE_SAEM, "Results/Decay_n20_ni5/SE_SAEM.csv", row.names = FALSE)
write.csv(SE_NLME, "Results/Decay_n20_ni5/SE_NLME.csv", row.names = FALSE)
write.csv(SE_LME4, "Results/Decay_n20_ni5/SE_LME4.csv", row.names = FALSE)
write.csv(D_SAEM, "Results/Decay_n20_ni5/D_SAEM.csv", row.names = FALSE)
write.csv(D_NLME, "Results/Decay_n20_ni5/D_NLME.csv", row.names = FALSE)
write.csv(D_LME4, "Results/Decay_n20_ni5/D_LME4.csv", row.names = FALSE)
write.csv(time_SAEM, "Results/Decay_n20_ni5/time_SAEM.csv", row.names = FALSE)
write.csv(time_NLME, "Results/Decay_n20_ni5/time_NLME.csv", row.names = FALSE)
write.csv(time_LME4, "Results/Decay_n20_ni5/time_LME4.csv", row.names = FALSE)
write.csv(sigma_SAEM, "Results/Decay_n20_ni5/sigma_SAEM.csv", row.names = FALSE)
write.csv(sigma_NLME, "Results/Decay_n20_ni5/sigma_NLME.csv", row.names = FALSE)
write.csv(sigma_LME4, "Results/Decay_n20_ni5/sigma_LME4.csv", row.names = FALSE)


#======================Calculate MSE and bias===================
# Read the results file
estimates_SAEM <- read.csv("Results/Decay_n20_ni5/estimates_SAEM.csv")
estimates_NLME <- read.csv("Results/Decay_n20_ni5/estimates_NLME.csv")
estimates_LME4 <- read.csv("Results/Decay_n20_ni5/estimates_LME4.csv")
SE_SAEM <- read.csv("Results/Decay_n20_ni5/SE_SAEM.csv")
SE_NLME <- read.csv("Results/Decay_n20_ni5/SE_NLME.csv")
SE_LME4 <- read.csv("Results/Decay_n20_ni5/SE_LME4.csv")
D_SAEM <- read.csv("Results/Decay_n20_ni5/D_SAEM.csv")
D_NLME <- read.csv("Results/Decay_n20_ni5/D_NLME.csv")
D_LME4 <- read.csv("Results/Decay_n20_ni5/D_LME4.csv")
time_SAEM <- read.csv("Results/Decay_n20_ni5/time_SAEM.csv")$x
time_NLME <- read.csv("Results/Decay_n20_ni5/time_NLME.csv")$x
time_LME4 <- read.csv("Results/Decay_n20_ni5/time_LME4.csv")$x
sigma_SAEM <- read.csv("Results/Decay_n20_ni5/sigma_SAEM.csv")$x
sigma_NLME <- read.csv("Results/Decay_n20_ni5/sigma_NLME.csv")$x
sigma_LME4 <- read.csv("Results/Decay_n20_ni5/sigma_LME4.csv")$x
...
for (i in seq_along(all_parameters)){
  rMSE_SAEM[i]=sqrt(sum((estimates_SAEM_all[,i]-all_parameters[i])^2)/num_sim)/all_parameters[i]
  bias_SAEM[i]=sum(estimates_SAEM_all[,i]-all_parameters[i])/num_sim
  bias_perc_SAEM[i]=bias_SAEM[i]/all_parameters[i]
}
Coverage_SAEM <- sapply(CI_SAEM, mean)
result_SAEM <- cbind(all_parameters, estimates = sapply(estimates_SAEM_all, mean), se_m = c(sapply(SE_SAEM, mean), rep(NA, 4)), se_s = sapply(estimates_SAEM_all, sd), rMSE_SAEM,bias_SAEM,bias_perc_SAEM, CP = c(Coverage_SAEM, rep(NA, 4)), NC = c(0, NA,NA, NA,NA, NA, NA), time = c(sum(time_SAEM), NA, NA, NA, NA, NA, NA))
...
D_NLME <- na.omit(D_NLME)
sigma_NLME <- na.omit(sigma_NLME)
...
for (i in seq_along(all_parameters)){
  rMSE_NLME[i]=sqrt(sum((estimates_NLME_all[,i]-all_parameters[i])^2)/nrow(estimates_NLME_all))/all_parameters[i]
  bias_NLME[i]=sum(estimates_NLME_all[,i]-all_parameters[i])/nrow(estimates_NLME_all)
  bias_perc_NLME[i]=bias_NLME[i]/all_parameters[i]
}
Coverage_NLME <- sapply(CI_NLME, mean)
result_NLME <- cbind(all_parameters,estimates = sapply(estimates_NLME_all, mean), se_m = c(sapply(SE_NLME, mean), rep(NA, 4)), se_s = sapply(estimates_NLME_all, sd), rMSE_NLME,bias_NLME,bias_perc_NLME, CP = c(Coverage_NLME, rep(NA, 4)), NC = c(num_sim-nrow(estimates_NLME), NA, NA, NA, NA, NA, NA), time = c(sum(na.omit(time_NLME)), NA, NA, NA, NA, NA, NA))
...
for (i in seq_along(all_parameters)){
  rMSE_LME4[i]=sqrt(sum((estimates_LME4_all[,i]-all_parameters[i])^2)/num_sim)/all_parameters[i]
  bias_LME4[i]=sum(estimates_LME4_all[,i]-all_parameters[i])/num_sim
  bias_perc_LME4[i]=bias_LME4[i]/all_parameters[i]
}
Coverage_LME4 <- sapply(CI_LME4, mean)
result_LME4 <- cbind(all_parameters,estimates = sapply(estimates_LME4_all, mean), se_m = c(sapply(SE_LME4, mean),rep(NA,4)), se_s = sapply(estimates_LME4_all, sd), rMSE_LME4,bias_LME4,bias_perc_LME4, CP = c(Coverage_LME4, rep(NA, 4)), NC = c(0, NA, NA, rep(NA, 4)), time = c(sum(time_LME4), NA, NA, rep(NA, 4)))
...
table$Parameter <- rep(c("$P_1$","$\\lambda_1$","$P_2$", "$D_{11}$", "$D_{22}$", "$D_{33}$", "$\\sigma$"), 3)
...
print(latex_table, file = "Table/Decay_n20_ni5.tex",include.rownames=FALSE,sanitize.text.function = function(x){x},hline.after = c(-1,0,21))
