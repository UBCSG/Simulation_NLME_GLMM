...
for (i in 2:100){
  ...
  write.table(data_sim,"Data/data_DecayIII.txt", sep = "," ,quote = FALSE,row.names = FALSE)
  
  # Fit viral decay model using Monolix.
  start.time <- Sys.time()
  data = list(dataFile = paste0('Data/data_DecayIII.txt'),
  ...
  start.time <- Sys.time()
  while(is.na(estimates_NLME[i, 1])){
    tryCatch({
      cat("Iteration:", i, "\n")
...
write.csv(estimates_SAEM, "Results/DecayIII/estimates_SAEM.csv", row.names = FALSE)
write.csv(estimates_NLME, "Results/DecayIII/estimates_NLME.csv", row.names = FALSE)
write.csv(estimates_LME4, "Results/DecayIII/estimates_LME4.csv", row.names = FALSE)
write.csv(SE_SAEM, "Results/DecayIII/SE_SAEM.csv", row.names = FALSE)
write.csv(SE_NLME, "Results/DecayIII/SE_NLME.csv", row.names = FALSE)
write.csv(SE_LME4, "Results/DecayIII/SE_LME4.csv", row.names = FALSE)
write.csv(time_SAEM, "Results/DecayIII/time_SAEM.csv", row.names = FALSE)
write.csv(time_NLME, "Results/DecayIII/time_NLME.csv", row.names = FALSE)
write.csv(time_LME4, "Results/DecayIII/time_LME4.csv", row.names = FALSE)


#======================Calculate MSE and bias===================
# Read the results file
estimates_SAEM <- read.csv("Results/DecayIII/estimates_SAEM.csv")
estimates_NLME <- read.csv("Results/DecayIII/estimates_NLME.csv")
estimates_LME4 <- read.csv("Results/DecayIII/estimates_LME4.csv")
SE_SAEM <- read.csv("Results/DecayIII/SE_SAEM.csv")
SE_NLME <- read.csv("Results/DecayIII/SE_NLME.csv")
SE_LME4 <- read.csv("Results/DecayIII/SE_LME4.csv")
time_SAEM <- read.csv("Results/DecayIII/time_SAEM.csv")
time_NLME <- read.csv("Results/DecayIII/time_NLME.csv")
time_LME4 <- read.csv("Results/DecayIII/time_LME4.csv")
...
result_LME4 <- cbind(true_value,estimates = sapply(estimates_LME4, mean), se_m = sapply(SE_LME4, mean), se_s = sapply(estimates_LME4, sd), rMSE_LME4,bias_LME4,bias_perc_LME4, Coverage_LME4, NC = c(0, NA, NA), time = c(sum(time_LME4), NA, NA))
...
digits(latex_table)<-c(0,3,1,0,1,1,3,3,3,3,3,2)
print(latex_table, file = "Table/DecayIII.tex",include.rownames=FALSE,sanitize.text.function = function(x){x},hline.after = c(-1,0,9))
