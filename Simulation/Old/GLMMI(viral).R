...
for (i in 1:num_sim){
  ...
  data_sim <- data %>% 
    ...
    mutate(xb = (alpha0 + a1_sim) + (alpha1 + a2_sim) * x) %>% 
    mutate(p = 1/(1+exp(-xb))) %>% 
    mutate(y = rbinom(n = nrow(data), size = 1, p = p)) %>% 
    dplyr::select(-a1_sim, -a2_sim, -xb, -p, -e, -day)
  ...
  data = list(dataFile = paste0('Data/data_GLMMI.txt'),
              headerTypes =c("id","regressor","observation"),
  ...
  start.time <- Sys.time()
  tryCatch({data_lme4 <- groupedData(y ~ x | PATIENT, data = data_sim)
  ...
  SE_LME4[i, ] <- summary(model_lme4)$coefficients[, 2]}, silent = TRUE)
