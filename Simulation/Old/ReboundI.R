...
beta4 = 1.9
beta5 = 0.4
B <- diag(c(0.2, 0, 0.1, 0, 0))

#Choose time from true_value dataset. 
# time <- c(0.5, 2.9, 4.8, 7, 10.2) #PATIENT #1
time <- c(0.5, 0.7, 1.2, 1.8, 2.3, 2.9, 3.3, 4.8, 7, 10.2)

#Create a data frame for simulation estimates.
estimates_SAEM <- estimates_NLME <- estimates_LME4 <- data.frame(matrix(nrow=num_sim,ncol = 6))
colnames(estimates_SAEM) <- colnames(estimates_NLME) <- colnames(estimates_LME4) <- c("beta1","beta2","beta3","beta4","beta5","warning")
...
CI_SAEM <- CI_NLME <- CI_LME4 <- data.frame(matrix(nrow=num_sim,ncol = 5))
colnames(CI_SAEM) <- colnames(CI_NLME) <- colnames(CI_LME4) <- c("beta1","beta2","beta3","beta4","beta5")

time_SAEM <- time_NLME <- time_LME4 <- c(rep(NA, num_sim))

Model <- function(beta1 ,beta2 ,beta3 ,beta4 ,beta5 ,t) {
  beta1 * t / (t  + exp( beta2 - beta3 * t)) + beta4 / (1 + exp( beta5 * t))
}
...
for (i in 1:num_sim){
  ...
  data_sim <- data %>% 
    mutate(viral = (beta1 + b1_sim) * day / (day  + exp( beta2 + b2_sim - (beta3 + b3_sim) * day)) + (beta4 + b4_sim)/ (1 + exp( (beta5 + b5_sim)* day))+e)%>% 
  ...
  write.table(data_sim,"Data/data_reboundI.txt", sep = "," ,quote = FALSE,row.names = FALSE)
  
  # Fit viral Rebound model using Monolix.
  start.time <- Sys.time()
  data = list(dataFile = paste0('Data/data_reboundI.txt'),
  ...
  setIndividualParameterVariability(beta1 = TRUE, beta2 = FALSE, beta3 = TRUE, beta4 = FALSE, beta5 = FALSE)
  setPopulationParameterInformation(beta1_pop = list(initialValue = 4), 
                                    beta2_pop = list(initialValue = 5.7), 
                                    beta3_pop = list(initialValue = 2.1), 
                                    beta4_pop = list(initialValue = 1.9), 
                                    beta5_pop = list(initialValue = 0.4),
                                    omega_beta1 = list(initialValue = 0.2),
                                    omega_beta3 = list(initialValue = 0.1),
                                    a = list(initialValue = 0.1))
  setIndividualParameterDistribution(beta1="normal", beta2="normal", beta3="normal", beta4="normal", beta5="normal")
  ...
  model_nlme <- nlme(viral ~ beta1 * day / (day  + exp( beta2 - beta3 * day)) + beta4 / (1 + exp( beta5 * day)),
                     fixed = beta1+beta2+beta3+beta4+beta5 ~ 1,
                     random = beta1+beta3 ~ 1,
                     data = data_nlme,
                     start = c(beta1 = 4, beta2 = 5.7, beta3 = 2.1, beta4 = 1.9, beta5 = 0.4))

  estimates_NLME[i, 1:5] <- summary(model_nlme)$tTable[,1]
  SE_NLME[i, ] <- summary(model_nlme)$tTable[,2]
  }, warning = function(w) {estimates_NLME[i, 6] <<- w
  estimates_NLME[i, 1:5] <<- summary(model_nlme)$tTable[,1]
  SE_NLME[i, ] <<- summary(model_nlme)$tTable[,2]},
  ...
  #   viral ~ 
  #     # Fixed effects
  #     ModelGradient(beta1, beta2, beta3, beta4, beta5, day) ~ 
  #     # Random effects
  #     (beta1 | PATIENT) + (beta3 | PATIENT), 
  #   # Data
  #   data = data_sim, 
  #   start =  c(beta1 = 4, beta2 = 5.7, beta3 = 2.1, beta4 = 1.9, beta5 = 0.4))
  # 
  ...
  # }, error = function(e){
  #     fail.LME4 = fail.LME4 + 1
  #     print("LME4 fail to converge")
  #   } 
  #   )
  # end.time <- Sys.time()
  ...
  nfun <- deriv(nform, namevec = c("beta1", "beta2", "beta3", "beta4", "beta5"), 
                function.arg = c("t", "beta1", "beta2", "beta3", "beta4", "beta5"))
  tryCatch({
    model_lme4 <- nlmer(viral ~ nfun(day, beta1, beta2, beta3, beta4, beta5) ~ (beta1 + beta3| PATIENT), data_nlme, start = c(beta1 = 12, beta2 = 5.7, beta3 = 2.1, beta4 = 1.9, beta5 = 0.4))
    estimates_LME4[i, 1:5] <- summary(model_lme4)$coefficients[, 1]
    SE_LME4[i, ] <- summary(model_lme4)$coefficients[, 2]
  }, warning = function(w) {estimates_LME4[i, 1:5] <<- summary(model_lme4)$coefficients[, 1]
  SE_LME4[i, ] <<- summary(model_lme4)$coefficients[, 2]
  estimates_LME4[i, 6] <<- w}, 
  error = function(e) {estimates_LME4[i, ] <<- SE_LME4[i, ] <<- NA})
  end.time <- Sys.time()
  time_LME4[i] <- end.time - start.time
  
  write.csv(estimates_SAEM, "Results/ReboundI/estimates_SAEM.csv", row.names = FALSE)
  write.csv(estimates_NLME, "Results/ReboundI/estimates_NLME.csv", row.names = FALSE)
  write.csv(estimates_LME4, "Results/ReboundI/estimates_LME4.csv", row.names = FALSE)
  write.csv(SE_SAEM, "Results/ReboundI/SE_SAEM.csv", row.names = FALSE)
  write.csv(SE_NLME, "Results/ReboundI/SE_NLME.csv", row.names = FALSE)
  write.csv(SE_LME4, "Results/ReboundI/SE_LME4.csv", row.names = FALSE)
  write.csv(time_SAEM, "Results/ReboundI/time_SAEM.csv", row.names = FALSE)
  write.csv(time_NLME, "Results/ReboundI/time_NLME.csv", row.names = FALSE)
  write.csv(time_LME4, "Results/ReboundI/time_LME4.csv", row.names = FALSE)
}  


#======================Calculate MSE and bias===================
# Read the results file
estimates_SAEM <- read.csv("Results/ReboundI/estimates_SAEM.csv")
estimates_NLME <- read.csv("Results/ReboundI/estimates_NLME.csv")
estimates_LME4 <- read.csv("Results/ReboundI/estimates_LME4.csv")
SE_SAEM <- read.csv("Results/ReboundI/SE_SAEM.csv")
SE_NLME <- read.csv("Results/ReboundI/SE_NLME.csv")
SE_LME4 <- read.csv("Results/ReboundI/SE_LME4.csv")
time_SAEM <- read.csv("Results/ReboundI/time_SAEM.csv")$x
time_NLME <- read.csv("Results/ReboundI/time_NLME.csv")$x
time_LME4 <- read.csv("Results/ReboundI/time_LME4.csv")$x

## warning
summary(as.factor(estimates_LME4$warning))
summary(as.factor(estimates_NLME$warning))
estimates_NLME <- estimates_NLME[which(is.na(estimates_NLME$warning)), ]
rownumber <- as.numeric(rownames(estimates_NLME))
SE_NLME <- SE_NLME[rownumber, ]

estimates_LME4 <- estimates_LME4[, -6]
estimates_NLME <- estimates_NLME[, -6]
estimates_SAEM <- estimates_SAEM[, -6]


true_value <- c(beta1, beta2, beta3, beta4, beta5)
...
for (i in 1:nrow(estimates_SAEM)){
...
Coverage_SAEM <- sapply(CI_SAEM, mean, na.rm = TRUE)
result_SAEM <- cbind(true_value, estimates = sapply(estimates_SAEM, mean), se_m = sapply(SE_SAEM, mean, na.rm = TRUE), se_s = sapply(estimates_SAEM, sd), rMSE_SAEM,bias_SAEM,bias_perc_SAEM, Coverage_SAEM, NC = c(num_sim - length(na.omit(SE_SAEM[, 1])), num_sim - length(na.omit(SE_SAEM[, 2])),num_sim - length(na.omit(SE_SAEM[, 3])), num_sim - length(na.omit(SE_SAEM[, 4])),num_sim - length(na.omit(SE_SAEM[, 5]))), time = c(sum(time_SAEM), NA,NA, NA, NA))
rownames(result_SAEM)=c("beta1","beta2","beta3","beta4","beta5")
...
result_NLME <- cbind(true_value,estimates = sapply(estimates_NLME, mean), se_m = sapply(SE_NLME, mean), se_s = sapply(estimates_NLME, sd), rMSE_NLME,bias_NLME,bias_perc_NLME, Coverage_NLME, NC = c(num_sim - nrow(estimates_NLME), NA, NA, NA, NA), time = c(sum(na.omit(time_NLME)), NA, NA, NA, NA))
rownames(result_NLME)=c("beta1","beta2","beta3","beta4","beta5")
...
result_LME4 <- cbind(true_value,estimates = sapply(estimates_LME4, mean), se_m = sapply(SE_LME4, mean), se_s = sapply(estimates_LME4, sd), rMSE_LME4,bias_LME4,bias_perc_LME4, Coverage_LME4, NC = c(num_sim - nrow(estimates_LME4), NA, NA, NA, NA), time = c(sum(na.omit(time_LME4)), NA, NA, NA, NA))
rownames(result_LME4)=c("beta1","beta2","beta3","beta4","beta5")

# Latex Table
table_names <- c( "Method", "Time (s)", "NC","Parameter","True Value", "Estimate", "SE_M",  "SE_S", "Bias (%)","rMSE (%)","Coverage")
table <- data.frame(matrix(nrow=15, ncol=length(table_names)))
colnames(table) <- table_names
table$Parameter <- rep(c("$\\beta_1$","$\\beta_2$","$\\beta_3$","$\\beta_4$","$\\beta_5$"), 3)
table$`True Value` <- rep(c(4, 5.7, 2.1, 1.9, 0.4),3)
table$Method <- c("SAEM",NA,NA,NA,NA,"NLME",NA,NA,NA,NA,"LME4",NA,NA,NA,NA)
...
digits(latex_table)<-c(0,2,2,0,1,1,2,2,2,2,2,2)
print(latex_table, file = "Table/ReboundV.tex",include.rownames=FALSE,sanitize.text.function = function(x){x},hline.after = c(-1,0,5,10,15))
