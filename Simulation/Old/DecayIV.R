...
for (i in 1:num_sim){
  ...
  write.table(data_sim,"Data/data_DecayIV.txt", sep = "," ,quote = FALSE,row.names = FALSE)
  
  # Fit viral decay model using Monolix.
  start.time <- Sys.time()
  data = list(dataFile = paste0('Data/data_DecayIV.txt'),
  ...
  model_lme4 <- nlmer(viral ~ nfun(day, p1, b1, p2) ~ p1 + p2 + b1| PATIENT, data_sim, start = c(p1 = 17, b1 = 4, p2 = 3))
  estimates_LME4[i, ] <- summary(model_lme4)$coefficients[, 1]
  SE_LME4[i, ] <- summary(model_lme4)$coefficients[, 2]
  end.time <- Sys.time()
  time_LME4[i] <- end.time - start.time
...
write.csv(estimates_SAEM, "Results/DecayIV/estimates_SAEM.csv", row.names = FALSE)
write.csv(estimates_NLME, "Results/DecayIV/estimates_NLME.csv", row.names = FALSE)
write.csv(estimates_LME4, "Results/DecayIV/estimates_LME4.csv", row.names = FALSE)
write.csv(SE_SAEM, "Results/DecayIV/SE_SAEM.csv", row.names = FALSE)
write.csv(SE_NLME, "Results/DecayIV/SE_NLME.csv", row.names = FALSE)
write.csv(SE_LME4, "Results/DecayIV/SE_LME4.csv", row.names = FALSE)
write.csv(time_SAEM, "Results/DecayIV/time_SAEM.csv", row.names = FALSE)
write.csv(time_NLME, "Results/DecayIV/time_NLME.csv", row.names = FALSE)
write.csv(time_LME4, "Results/DecayIV/time_LME4.csv", row.names = FALSE)


#======================Calculate MSE and bias===================
# Read the results file
estimates_SAEM <- read.csv("Results/DecayIV/estimates_SAEM.csv")
estimates_NLME <- read.csv("Results/DecayIV/estimates_NLME.csv")
estimates_LME4 <- read.csv("Results/DecayIV/estimates_LME4.csv")
SE_SAEM <- read.csv("Results/DecayIV/SE_SAEM.csv")
SE_NLME <- read.csv("Results/DecayIV/SE_NLME.csv")
SE_LME4 <- read.csv("Results/DecayIV/SE_LME4.csv")
time_SAEM <- read.csv("Results/DecayIV/time_SAEM.csv")$x
time_NLME <- read.csv("Results/DecayIV/time_NLME.csv")$x
time_LME4 <- read.csv("Results/DecayIV/time_LME4.csv")$x
...
print(latex_table, file = "Table/DecayIV.tex",include.rownames=FALSE,sanitize.text.function = function(x){x},hline.after = c(-1,0,9))
