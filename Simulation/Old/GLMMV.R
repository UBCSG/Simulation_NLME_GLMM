...
time <- c(0.5, 1.67, 3, 4.6, 6)
# time <- c(0.5,1.67,3, 4.6, 6, 7.8, 9, 10.3, 11.2, 12.6) #PATIENT #1
...
for (i in 1:num_sim){
  ...
  # simulate a_i
  a1_sim <- rnorm(num_patient, 0, sigma)
  a1_sim <- cbind(PATIENT=c(1:num_patient),a1_sim)
  data <- merge(data, a1_sim, by="PATIENT")

  data_sim <- data %>% 
    mutate(xb = (alpha0 + a1_sim) + alpha1 * day) %>% 
    mutate(p = 1/(1+exp(-xb))) %>% 
    mutate(y = rbinom(n = nrow(data), size = 1, p = p)) %>% 
    dplyr::select(-a1_sim, -xb, -p)
  summary(as.factor(data_sim$y))

  # save the simulated data locally
  write.table(data_sim,"Data/data_GLMMV.txt", sep = "," ,quote = FALSE,row.names = FALSE)
  
  # Fit viral decay model using Monolix.
  start.time <- Sys.time()
  data = list(dataFile = paste0('Data/data_GLMMV.txt'),
  ...
  setIndividualParameterVariability(alpha0 = TRUE, alpha1 = FALSE)
  ...
  write.csv(estimates_SAEM, "Results/GLMMV/estimates_SAEM.csv", row.names = FALSE)
  write.csv(estimates_LME4, "Results/GLMMV/estimates_LME4.csv", row.names = FALSE)
  write.csv(SE_SAEM, "Results/GLMMV/SE_SAEM.csv", row.names = FALSE)
  write.csv(SE_LME4, "Results/GLMMV/SE_LME4.csv", row.names = FALSE)
  write.csv(time_SAEM, "Results/GLMMV/time_SAEM.csv", row.names = FALSE)
  write.csv(time_LME4, "Results/GLMMV/time_LME4.csv", row.names = FALSE)
}  
...
estimates_SAEM <- read.csv("Results/GLMMV/estimates_SAEM.csv")
estimates_LME4 <- read.csv("Results/GLMMV/estimates_LME4.csv")
SE_SAEM <- read.csv("Results/GLMMV/SE_SAEM.csv")
SE_LME4 <- read.csv("Results/GLMMV/SE_LME4.csv")
time_SAEM <- read.csv("Results/GLMMV/time_SAEM.csv")
time_LME4 <- read.csv("Results/GLMMV/time_LME4.csv")
...
print(latex_table, file = "Table/GLMMV4.tex",include.rownames=FALSE,sanitize.text.function = function(x){x},hline.after = c(-1,0,4))
