...
for (i in 1:num_sim){
  ...
  write.table(data_sim,"Data/data_reboundIV.txt", sep = "," ,quote = FALSE,row.names = FALSE)
  
  # Fit viral Rebound model using Monolix.
  start.time <- Sys.time()
  data = list(dataFile = paste0('Data/data_reboundIV.txt'),
  ...
  write.csv(estimates_SAEM, "Results/ReboundIV/estimates_SAEM.csv", row.names = FALSE)
  write.csv(estimates_NLME, "Results/ReboundIV/estimates_NLME.csv", row.names = FALSE)
  write.csv(estimates_LME4, "Results/ReboundIV/estimates_LME4.csv", row.names = FALSE)
  write.csv(SE_SAEM, "Results/ReboundIV/SE_SAEM.csv", row.names = FALSE)
  write.csv(SE_NLME, "Results/ReboundIV/SE_NLME.csv", row.names = FALSE)
  write.csv(SE_LME4, "Results/ReboundIV/SE_LME4.csv", row.names = FALSE)
  write.csv(time_SAEM, "Results/ReboundIV/time_SAEM.csv", row.names = FALSE)
  write.csv(time_NLME, "Results/ReboundIV/time_NLME.csv", row.names = FALSE)
  write.csv(time_LME4, "Results/ReboundIV/time_LME4.csv", row.names = FALSE)
}  


#======================Calculate MSE and bias===================
# Read the results file
estimates_SAEM <- read.csv("Results/ReboundIV/estimates_SAEM.csv")
estimates_NLME <- read.csv("Results/ReboundIV/estimates_NLME.csv")
estimates_LME4 <- read.csv("Results/ReboundIV/estimates_LME4.csv")
SE_SAEM <- read.csv("Results/ReboundIV/SE_SAEM.csv")
SE_NLME <- read.csv("Results/ReboundIV/SE_NLME.csv")
SE_LME4 <- read.csv("Results/ReboundIV/SE_LME4.csv")
time_SAEM <- read.csv("Results/ReboundIV/time_SAEM.csv")$x
time_NLME <- read.csv("Results/ReboundIV/time_NLME.csv")$x
time_LME4 <- read.csv("Results/ReboundIV/time_LME4.csv")$x
...
result_SAEM <- cbind(true_value, estimates = sapply(estimates_SAEM, mean), se_m = sapply(SE_SAEM, mean), se_s = sapply(estimates_SAEM, sd), rMSE_SAEM,bias_SAEM,bias_perc_SAEM, Coverage_SAEM, NC = c(0, NA,NA, NA,NA), time = c(sum(time_SAEM), NA,NA, NA, NA))
...
print(latex_table, file = "Table/ReboundIV.tex",include.rownames=FALSE,sanitize.text.function = function(x){x},hline.after = c(-1,0,5,10,15))
