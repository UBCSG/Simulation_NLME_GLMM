...
for (i in 4:num_sim){
  ...
  data_sim$y <- as.factor(data_sim$y)
  summary(as.factor(data_sim$y))

  # save the simulated data locally
  write.table(data_sim,"Data/data_GLMMII.txt", sep = "," ,quote = FALSE,row.names = FALSE)
  
  # Fit viral decay model using Monolix.
  start.time <- Sys.time()
  data = list(dataFile = paste0('Data/data_GLMMII.txt'),
...
write.csv(estimates_SAEM, "Results/GLMMII/estimates_SAEM.csv", row.names = FALSE)
write.csv(estimates_LME4, "Results/GLMMII/estimates_LME4.csv", row.names = FALSE)
write.csv(SE_SAEM, "Results/GLMMII/SE_SAEM.csv", row.names = FALSE)
write.csv(SE_LME4, "Results/GLMMII/SE_LME4.csv", row.names = FALSE)
write.csv(time_SAEM, "Results/GLMMII/time_SAEM.csv", row.names = FALSE)
write.csv(time_LME4, "Results/GLMMII/time_LME4.csv", row.names = FALSE)


#======================Calculate MSE and bias===================
# Read the results file
estimates_SAEM <- read.csv("Results/GLMMII/estimates_SAEM.csv")
estimates_LME4 <- read.csv("Results/GLMMII/estimates_LME4.csv")
SE_SAEM <- read.csv("Results/GLMMII/SE_SAEM.csv")
SE_LME4 <- read.csv("Results/GLMMII/SE_LME4.csv")
time_SAEM <- read.csv("Results/GLMMII/time_SAEM.csv")
time_LME4 <- read.csv("Results/GLMMII/time_LME4.csv")
...
print(latex_table, file = "Table/GLMMII.tex",include.rownames=FALSE,sanitize.text.function = function(x){x},hline.after = c(-1,0,4))
