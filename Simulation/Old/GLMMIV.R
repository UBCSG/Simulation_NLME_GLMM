...
for (i in 2:num_sim){
  ...
  write.table(data_sim,"Data/data_GLMMIV.txt", sep = "," ,quote = FALSE,row.names = FALSE)
  
  # Fit viral decay model using Monolix.
  start.time <- Sys.time()
  data = list(dataFile = paste0('Data/data_GLMMIV.txt'),
...
write.csv(estimates_SAEM, "Results/GLMMIV/estimates_SAEM.csv", row.names = FALSE)
write.csv(estimates_LME4, "Results/GLMMIV/estimates_LME4.csv", row.names = FALSE)
write.csv(SE_SAEM, "Results/GLMMIV/SE_SAEM.csv", row.names = FALSE)
write.csv(SE_LME4, "Results/GLMMIV/SE_LME4.csv", row.names = FALSE)
write.csv(time_SAEM, "Results/GLMMIV/time_SAEM.csv", row.names = FALSE)
write.csv(time_LME4, "Results/GLMMIV/time_LME4.csv", row.names = FALSE)


#======================Calculate MSE and bias===================
# Read the results file
estimates_SAEM <- read.csv("Results/GLMMIV/estimates_SAEM.csv")
estimates_LME4 <- read.csv("Results/GLMMIV/estimates_LME4.csv")
SE_SAEM <- read.csv("Results/GLMMIV/SE_SAEM.csv")
SE_LME4 <- read.csv("Results/GLMMIV/SE_LME4.csv")
time_SAEM <- read.csv("Results/GLMMIV/time_SAEM.csv")$x
time_LME4 <- read.csv("Results/GLMMIV/time_LME4.csv")$x
...
print(latex_table, file = "Table/GLMMIV.tex",include.rownames=FALSE,sanitize.text.function = function(x){x},hline.after = c(-1,0,4))
